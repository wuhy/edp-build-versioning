 ...
 * @param {Object} processor 版本号处理器
 * @return {Object} key为给定深度的路径前缀，value为对应该路径前缀深度的md5版本号
 */
function collectPathVersion(root, processor) {
    var depth = processor.pathPrefixDepth;
    var sourceRootRegExp = processor.sourceRootRegExp;
    var md5Length = processor.md5Length;
    var pathVersionMap  = {};
...
module.exports = exports = function (versionProcessor, souceFiles) {
    ...
    if (!outputOption) {
        outputOption = '__edp_build_versiong_internal__';
    }
    ...
    if (versionGenerator) {
        outputVersionInfo = versionGenerator.call(versionProcessor, processFiles);
    }
    else {
        // 生成路径前缀对应的版本号映射信息
        ...
        outputVersionInfo = collectPathVersion(
            md5VersionTree, versionProcessor
        );

        // 对于非按页面输出情况，只输出指定的模块 id 的版本号信息
        var versionModuleIds = versionProcessor.moduleIds;
        if (!versionProcessor.requireOutputByPage && versionModuleIds) {
            var result = {};
    ...
    if (outputFile) {
        ...
        outputFile.data = 'require.config({ urlArgs: '
            + util.stringify(outputVersionInfo) + ' });';
        versionMap[outputFile.path] = util.md5sum(outputFile, versionProcessor.md5Length);
    }
        ...
        versionMap[outputOption] = {
            outputByPage: versionProcessor.requireOutputByPage,
